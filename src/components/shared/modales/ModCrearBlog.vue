...
const createBlog = async () => {
    try {
        ...
        formData.append('title', blogTittle.value);
        formData.append('text', blogText.value);
        if (image1.value) {
        ...
        await usersApi.post('/manager/blogs/', formData, {
            headers: {
                Authorization: `Bearer ${decryptDatos(localStorage.getItem('token') || '')}`
            }
        })
        showToast('Blog creado', 'success', 3000, POSITION.BOTTOM_RIGHT)
        props.onClick();
    } catch (error: any) {
        ...
        showToast('Error creando el blog.', 'error', 3000, POSITION.BOTTOM_RIGHT);
        }
    }
}
    ...
    <div class="w-[100vw] h-[100vh] absolute top-0 left-0 flex justify-center items-center bg-[#16161633] p-4">
        <div class="p-4 pb-6 rounded-3xl min-w-[900px] min-h-[600px]" :class="`bg-white`">
            ...
            <div class="flex justify-center">
                <button class="px-4 mt-4 bg-green-800 pb-1 rounded-[35px] text-white" @click="createBlog">{{props.butText}}</button>
